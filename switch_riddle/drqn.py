"""
Switch Riddle — DRQN + BC预热 + R2D2隐状态存储
FC特征提取 → LSTM记忆 → Q值输出
BC: ClassicStrategy监督学习预热网络
R2D2: replay buffer存储LSTM隐状态，采样时用作BPTT起点
"""
...
class BCBuffer:
    ...
    def push(self, obs: np.ndarray, expert_action: int):
        self.buffer.append((obs.copy(), expert_action))

    def sample(self, batch_size: int):
        batch = random.sample(self.buffer, batch_size)
        obs, actions = zip(*batch)
        return torch.FloatTensor(np.array(obs)), torch.LongTensor(actions)
...
# ============================================================
# 序列经验回放缓冲区（R2D2：存储隐状态）
# ============================================================
class SequenceReplayBuffer:
    """每个episode是一个list of (obs, action, reward, next_obs, done, h, c)"""
...
class DRQNTrainer:
    def __init__(
        ...
        self.q_network = DRQNetwork(obs_dim, action_dim, lstm_hidden, fc_dims).to(self.device)
        self.target_network = DRQNetwork(obs_dim, action_dim, lstm_hidden, fc_dims).to(self.device)
        self.target_network.load_state_dict(self.q_network.state_dict())

        self.optimizer = optim.Adam(self.q_network.parameters(), lr=lr)
        self.bc_optimizer = optim.Adam(self.q_network.parameters(), lr=bc_lr)
        self.loss_fn = nn.SmoothL1Loss()
        self.bc_loss_fn = nn.CrossEntropyLoss()

        self.bc_buffer = BCBuffer()
    ...
    def select_action(self, obs: np.ndarray, epsilon: Optional[float] = None) -> int:
        ...
        if random.random() < eps:
            with torch.no_grad():
                _, self.hidden_state = self.q_network(obs_tensor, self.hidden_state)
            return random.randint(0, self.action_dim - 1)

        with torch.no_grad():
            q_values, self.hidden_state = self.q_network(obs_tensor, self.hidden_state)
        return int(q_values.squeeze().argmax().item())

    def bc_train_step(self) -> Optional[float]:
        if len(self.bc_buffer) < self.batch_size:
            return None

        obs, expert_actions = self.bc_buffer.sample(self.batch_size)
        obs = obs.to(self.device)
        expert_actions = expert_actions.to(self.device)

        obs_seq = obs.unsqueeze(1)  # (batch, 1, obs_dim)
        h0 = self._init_hidden(self.batch_size)

        q_values, _ = self.q_network(obs_seq, h0)  # (batch, 1, action_dim)
        q_values = q_values.squeeze(1)  # (batch, action_dim)

        loss = self.bc_loss_fn(q_values, expert_actions)
    ...
    def bc_sequence_train_step(self) -> Optional[float]:
        """序列级BC：用存储的隐状态跑LSTM序列，CE loss 对专家动作"""
        if len(self.success_buffer) < self.batch_size:
            return None

        o, a, _r, _no, _dn, h0, c0 = self.success_buffer.sample(self.batch_size)
        obs_seq = o.to(self.device)
        expert_actions = a.to(self.device)
        h0 = h0.to(self.device)
        c0 = c0.to(self.device)

        q_values, _ = self.q_network(obs_seq, (h0, c0))  # (batch, seq_len, action_dim)
        batch, seq_len = q_values.shape[:2]
        q_flat = q_values.reshape(batch * seq_len, self.action_dim)
        act_flat = expert_actions.reshape(batch * seq_len)

        loss = self.bc_loss_fn(q_flat, act_flat)
    ...
    def train_step(self) -> Optional[float]:
        ...
        q_values, _ = self.q_network(obs_seq, (h0, c0))
        q_value = q_values.gather(2, act_seq.unsqueeze(2)).squeeze(2)

        with torch.no_grad():
            next_q_values, _ = self.target_network(nobs_seq, (h0, c0))
...
def _collect_transition(
    ...
    return (obs.copy(), action, reward, next_obs.copy(), done, h_np, c_np)


def bc_warmup_episode(env: SwitchRiddleEnv, trainer: DRQNTrainer) -> float:
    """BC预热：ClassicStrategy走episode，填充BC缓冲+序列缓冲（含隐状态）"""
    ...
    while env.agents:
        ...
        trainer.bc_buffer.push(obs, expert_action)

        # 跑一次DRQN前向，维护隐状态（但不使用DRQN的动作）
        _ = trainer.select_action(obs, epsilon=0.0)
        ...
        episode_transitions.append(
            _collect_transition(obs, expert_action, reward, next_obs, float(done), trainer)
        )
...
def train_episode(env: SwitchRiddleEnv, trainer: DRQNTrainer) -> float:
    """DRQN训练episode，存储隐状态以支持R2D2"""
    observations, _ = env.reset()
    trainer.reset_hidden()
    episode_reward = 0.0
    obs_dim = env.observation_spaces[env.possible_agents[0]].shape[0]
    ...
    while env.agents:
        ...
        episode_transitions.append(
            _collect_transition(obs, action, reward, next_obs, float(done), trainer)
        )
        observations = next_observations
...
def run_demo(env: SwitchRiddleEnv, trainer: DRQNTrainer) -> float:
    import pygame
...
def main():
    ...
    SAVE_PATH = "checkpoints/switch_riddle_drqn2.pt"

    print(f"设备: {'CUDA' if torch.cuda.is_available() else 'CPU'}")
    print(f"智能体: {NUM_AGENTS} | LSTM: {LSTM_HIDDEN} | 序列: {SEQ_LEN}")
    ...
    import datetime
    log_dir = f"runs/switch_riddle_drqn2_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}"
    ...
    for ep in range(1, WARMUP_EPISODES + 1):
        r = bc_warmup_episode(train_env, trainer)
        if r > 0:
            warmup_success += 1
        if ep % 50 == 0:
            print(
    ...
    # ---------- DRQN 训练 ----------
    print(f"\n开始 R2D2 训练 ({NUM_EPISODES} episodes)...\n")
